...
import CategoryDropdown, { type ShopCategoryFilterRow } from "./CategoryDropdown";
import ColorsDropdwon from "./ColorsDropdwon";
import PriceDropdown from "./PriceDropdown";
import ShopProducerFilter from "./ShopProducerFilter";
...
const ShopWithSidebar = () => {
  ...
  const [sortOption, setSortOption] = useState<SelectOption>(SORT_OPTIONS[0]);
  const sortValue = sortOption.value as ShopSortValue;

  const [categoryRows, setCategoryRows] = useState<ShopCategoryFilterRow[]>([]);
  const [productTypeId, setProductTypeId] = useState<number | null>(null);
  const [producerId, setProducerId] = useState<number | null>(null);
  const [brandKeyword, setBrandKeyword] = useState<string | null>(null);
  ...
  const allowedTypeIds = useMemo(() => new Set(categoryRows.map((c) => c.id)), [categoryRows]);
  ...
  useEffect(() => {
    ...
    if (!raw) return;
    const n = Number(raw);
    if (!Number.isInteger(n) || n < 1 || !allowedTypeIds.has(n)) return;
    setProductTypeId(n);
  }, [searchParams, allowedTypeIds, categoryRows.length]);
  ...
  const filters: ShopCatalogFilters = useMemo(
    () => ({
      productTypeId,
      producerId,
    ...
    [productTypeId, producerId, brandKeyword, minPrice, maxPrice, color]
  );

  const { products, page, setPage, totalElements, totalPages, loading, error, rangeLabel } =
    useShopCatalog(sortValue, filters);

  const selectedSortForSelect = useMemo(
    () => SORT_OPTIONS.find((o) => o.value === sortOption.value) ?? SORT_OPTIONS[0],
    [sortOption.value]
  );
  ...
  const resetFilters = () => {
    ...
    setProducerId(null);
    setBrandKeyword(null);
    setMinPrice(null);
    setMaxPrice(null);
    setColor(null);
    setPage(0);
    router.replace(pathname || "/shop-with-sidebar", { scroll: false });
  };
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      if (t instanceof Element && !t.closest(".sidebar-content")) {
        ...
        <div className="site-container">
          <div className="flex gap-7.5">
            ...
            <div className="xl:max-w-[870px] w-full">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    <p className="text-sm sm:text-base">
                      {loading ? (
                        <span className="text-dark-4">Đang tải…</span>
                      ) : (
                        <>
                          Hiển thị{" "}
                          <span className="text-dark">
                            {totalElements === 0 ? 0 : `${rangeLabel.from}–${rangeLabel.to}`}
                          </span>{" "}
                          / <span className="text-dark">{totalElements}</span> sản phẩm
                        </>
                      )}
                    </p>
                  </div>
              ...
              {error ? (
                <p className="text-red text-sm mb-4 px-1" role="alert">
                  {error}
                </p>
              ) : null}
              ...
              >
                {!loading && products.length === 0 ? (
                  <p className="col-span-full text-center text-dark-4 py-10">Chưa có sản phẩm.</p>
                ) : null}
                {products.map((item) =>
                  productStyle === "grid" ? (
